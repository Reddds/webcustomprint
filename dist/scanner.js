...
class Scanner {
    ...
    async AddToBase(scanned, prodInfo) {
        if (this.dbPool) {
            ...
            if (isIndividual) {
                ...
                const [rowsExist, fieldsExist] = await dbCon.execute(`SELECT * FROM prods WHERE code='${post.code}'`);
            ...
            if (!isExists) {
                ...
                newCats.forEach(async (catNew) => {
                    const [rows, fields] = await dbCon.execute(`SELECT * FROM catalog_categories WHERE cat_id=${catNew.cat_id}`);
                ...
                if (existRow && !existRow.ChesZnakDump) {
                    ...
                    await dbCon.query(`UPDATE prods SET ? WHERE ?`, [{ ChesZnakDump: JSON.stringify(prodInfo.Dump) }, { id: existRow.id }]);
                }
    ...
    async OnSacnned(data, format) {
        ...
        try {
            const prodInfo = await chestznak_1.ChesZnak.GetData(data);
            if (prodInfo) {
    ...
    async GetExistProds() {
        try {
            const [rowsExist, fieldsExist] = await this.dbPool.execute(`SELECT * FROM prods WHERE is_trashed=0 ORDER BY exp_date IS NULL, exp_date`);
    ...
    */
    ...
    async InitMysql() {
        try {
            ...
            // });
            this.dbPool = await promise_1.default.createPool({
                connectionLimit: 20,
                database: 'prods',
                host: "localhost",
                user: process.env.DB_LOGIN,
