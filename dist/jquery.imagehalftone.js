        ...
        var Halftone = function($img) {
            return {
                create: function() {
                    ...
                    // If set maxwidth or maxHeight
                    if (settings.maxWidth > 0 || settings.maxHeight > 0) {
                        let allScale = 1;
                        let scaleX = 1;
                        let sacleY = 1;
                        if (settings.maxWidth > 0) {
                            scaleX = imageWidth / settings.maxWidth;
                        }
                        if (settings.maxHeight > 0) {
                            sacleY = imageHeight / settings.maxHeight;
                        }
                        allScale = scaleX > sacleY ? scaleX : sacleY;

                        imageWidth /= allScale;
                        imageHeight /= allScale;
                    }
                    ...
                    this.ctx.drawImage($img[0], 0, 0, imageWidth, imageHeight);
                    ...
                    settings.target ?
                        (typeof settings.target === 'function' ? settings.target($el, this.raw) : $(settings.target).html($el)) :
                        $img.replaceWith($el);
                },
