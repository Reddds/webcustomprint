...
interface IEditProd {
    ...
    /** Единицы измерения в списке покупок */
    qu_id_purchase?: number | undefined;
    /** Единицы измерения в магазине */
    qu_id_stock?: number | undefined;
    /** Место хранения */
    location_id?: number | undefined;
    picture_file_name?: string;
}

interface IEditProdInList {
    amount?: number;
}

interface ICreateResponce {
    created_object_id: number;
}

interface IEditUserfieldsProd {
    alias?: string,
    thumb_image?: string,
    add_part?: number
}

interface IShopProd extends IEditProd {
    id: number;
    // name: string;
    // product_group_id: number;
    image: string;
    thumbimage: string;
    // qu_id_purchase: number | undefined;
    userfields: IProdUserFields;
}

interface IQUUserFields {
    add_part_variants: string
}
...
type prodModel = {
     ...
     * Единицы измерения
     * */
    addCountTypeId?: number,
    addCountType?: string,
    addCountPluralForms?: string,
    /** Доля единиц измерения */
    addCountPart?: number,
    /** Человекочитаемое количество */
    amount: string
}
...
async function getProdGroups(): Promise<IShopCategory[]> {
    try {
...
async function getLastShoppingList(): Promise<IShopList> {
    try {
        ...
        const lastShopList: IShopList = (<IShopList>await response.json())?.[0];
...
async function getShoppingListProds(idShopList: number): Promise<IShopListProds[]> {
    try {
        const params = new URLSearchParams({
        ...
        const lastShopListProds: IShopListProds[] = await response.json();
...
async function getShoppingListProd(idProdInList: number): Promise<IShopListProds> {
    try {
        ...
        const shopListProd: IShopListProds = await response.json();
...
async function deleteShoppingListProd(idProdInList: number): Promise<boolean> {
    try {
        const response = await fetch(`${GROCY_URL}/api/objects/shopping_list/${idProdInList}`,
            {
                method: "DELETE",
...
async function addShoppingListProd(shopListId: number, prodId: number, amount: number, quId: number):
    Promise<number> {
    try {
        ...
        const shopListProdRes: ICreateResponce = await response.json();
...
async function getProds(): Promise<IShopProd[]> {
    try {
...
async function getProd(prodId: number): Promise<IShopProd> {
    try {
...
async function getQUs(): Promise<IQU[]> {
    try {
        const params = new URLSearchParams({
            "query[]": `active=1`,
        });
...
async function getQU(id: number): Promise<IQU> {
    try {
...
async function getProdsInGroup(groupId: number): Promise<IShopProd[]> {
    try {
        const params = new URLSearchParams({
...
async function editProd(id: number, prodNewData: IEditProd): Promise<boolean> {
    try {
...
async function addProd(prodNewData: IEditProd): Promise<number> {
    try {
        const response = await fetch(`${GROCY_URL}/api/objects/products`,
            {
                method: "POST",
        ...
        const addProdRes: ICreateResponce = await response.json();
...
async function editProdInList(id: number, prodInListNewData: IEditProdInList): Promise<boolean> {
    try {
...
function getTimestampFilename(prefix: string, extension: string = 'txt') {
    const now = new Date();
...
function toFilenameSafe(str: string): string {
    if (!str) return '';
...
async function uploadBase64Manual(base64String: string, filename: string, fileGroup: string): Promise<boolean> {
...
function amountToHuman(amount: number, name: string, pluralForms: string): string {
    const n = amount;
...
async function editProdUserFields(id: number, prodUserFieldsNewData: IEditUserfieldsProd): Promise<boolean> {
    try {
...
*/
    ...
    const prodsByGroups: IShopProdByGroup[] =  //await dbPool.query<IShopProdByGroup[]>(`SELECT * FROM shopping_prods_by_groups`);
...
function fillProdModel(shoppingListProds: IShopListProds[], qus: IQU[], p: IShopProd) {
    // В списке покупок может быть несколько позиций с одинаковым товаром
    // Это не норма, но может быть для разных магазов. 
    // TODO: Потом сделать какой-нить фильтр
    // Пока берём первое вхождение
...
router.post('/addone', async (req, res, next) => {
    ...
    if (!prodInList) {
        ...
        const editData: IEditProdInList = {
            amount: newAmount
        };
...
router.post('/addedit', async (req, res, next) => {
    ...
    try {
        ...
        const addCountPart = sanitize(req.body.addCountPart);
        ...
        const userFields: IEditUserfieldsProd = {};
...
async function getTextFromUrl(url: string) {
    ...
    if (!resReq.ok) throw new Error(`HTTP ${resReq.status}`);
