...
function PrintImageCuctom(buf: Uint8Array) {
 ...
 * @returns Массив
 */
export function strToNumArr(str: string): Uint8Array {
    const retArr = [];
...
function PrintImageCitizen(buf: Uint8Array) {
    ...
    */
    ...
    const imageArr: number[] = [height >> 8, height & 0xFF];
    ...
    fs.writeFileSync(savedFile, bufWithCut);
...
export function PrintRawStr(raw: string) {
...
export function PrintRawBuf(raw: Uint8Array) {
    console.log("printing raw buffer...");
    //const fullBuf = new Uint8Array(Buffer.from(raw));
    //const bufWithCut = fullBuf;// new Uint8Array([...fullBuf, ...cutAndEject]); // 
    //console.log("bufWithCut", bufWithCut);
    fs.writeFileSync(printerFile, raw);
}
...
export function PrintText(textForPrint: string, forPrint: printedModel) {
    ...
    const textBytes = cp866buffer.encode(ReplaceSpecialChars(textForPrint));
